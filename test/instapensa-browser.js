...
(async function () {
    ...
    fill('[data-bind="material.text"]', 'Texto de apoio de teste #educação');
    fill('[data-bind="material.title"]', 'Leitura crítica');
    fill('#questions', 'Quem publicou?\nQual é a intenção?');
    click('#editor-back');
    await until(() => document.body.dataset.view === 'library');
    const saved = await app.db.getFeed(id);
    assert(saved.name === 'InstaPensa — teste de persistência' && saved.posts[0].material.text.includes('#educação'), 'Salva alterações ao sair imediatamente do editor');
    app.app.go('#/editar/' + id);
    await until(() => app.editor.current());
    assert(document.querySelector('#questions').value.includes('Qual é a intenção?'), 'Reabre a reflexão salva');
    ...
    await wait(600);
    const curtido = await app.db.getFeed(id);
    assert(curtido.posts[1].liked === true, 'A curtida dada em aula fica gravada no feed');
    click('#material-zoom');
